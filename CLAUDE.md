...
- **Fase 4 is computed but not promoted**: the `compatibilidade` table and `recalcular_compatibilidade()` function exist and have been run once, tagged `versao_formula = 'v2-teste'`. Validated (see the Compatibilidade section below) — ordering flips correctly, sample-size brake works. Not wired into `index.html`, not promoted to a permanent tag. **Next up: promote to tag `v2`** (no formula changes expected, just re-running under the real tag) and continue from there — pg_cron still intentionally not enabled, this stays a by-hand recalc.
